...
class Cube:
    ...
    #     interactive_cube = InteractiveCube(self)
    #     fig.add_axes(interactive_cube)
    #     return interactive_cube
    ...
    def on_frame(self, controller):
        ...
        if len(frame.hands) < 2:
            ...
            if hand.palm_normal.y > 0:
                if v[0]>v_threshold:
                    #print "'right', ",v[0],
                    interactive_cube.rotate_event('right',v[0]/3)
                elif v[0]<-v_threshold:
                    #print "'left', ",v[0],
                    interactive_cube.rotate_event('left',-v[0]/3)

                if v[1]>v_threshold:
                    #print "'up', ",v[1],
                    interactive_cube.rotate_event('up',v[1]/3)
                elif v[1]<-v_threshold:
                    #print "'down', ",v[1],
                    interactive_cube.rotate_event('down',-v[1]/3)
            ...
            for gesture in frame.gestures():
                ...
                # Determine clock direction using the angle between the pointable and the circle normal
                    ...
                    if gesture.state == Leap.Gesture.STATE_STOP:
                        ...
                        if hand.is_right:
                            interactive_cube.rotate_face('R',direction)
                        else:
                            interactive_cube.rotate_face('L',direction)
        else:
            for gesture in frame.gestures():
                if gesture.type == Leap.Gesture.TYPE_CIRCLE:
                    circle = CircleGesture(gesture)
                    prev_id = gesture.id
                hand = circle.pointable.hand
                ...
                if gesture.state == Leap.Gesture.STATE_STOP:
                    ...
                    if (hand.is_right):
                        interactive_cube.rotate_face('U',direction)
                    else:
                        interactive_cube.rotate_face('D',direction)
...
if __name__ == '__main__':
    ...
    #interactive_cube = c.draw_interactive()
    fig = plt.figure(figsize=(5, 5))
    interactive_cube = InteractiveCube(c)
    fig.add_axes(interactive_cube)
    #print interactive_cube.__dict__
